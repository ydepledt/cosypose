...
def main():
    # path initialization
    data_path = LOCAL_DATA_DIR / 'demo_imu' 
    ...
    cam_infos = json.loads((data_path / 'realsense.json').read_text())
    ...
    for i in range (1,number_image):
        ...
        # init formatting
        df_frame = df_init.loc[df_init['id']==i].reset_index()
        n_detections = df_frame.shape[0]
        bboxes = torch.zeros(n_detections,4)
        batch_im_id = []
        label = []
        score = []

        for k in range(n_detections):
            # bboxes[k,:] = torch.tensor(df_frame['bbox'][k])
            batch_im_id.append(0)
            label.append(df_frame['object_id'][k])
            score.append(1)

        infos = pd.DataFrame({'batch_im_id': batch_im_id,
                'label': label,
                'score': score})
        ...
        wolf_poses = torch.tensor(poses_frame).float().cuda()
        detections = tc.PandasTensorCollection(bboxes=bboxes.float().cuda(), infos=infos)
        wolf_preds = tc.PandasTensorCollection(poses=wolf_poses, infos=infos)
        ...
        if video_render:
            ...
            figures = sv_viewer.make_singleview_custom_plot(rgb, camera,
                    renderer, wolf_preds, detections)
